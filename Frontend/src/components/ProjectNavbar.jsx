...
const ProjectNavbar = ({ onSearch }) => {
  ...
  // ✅ On mount, restore searchTerm from localStorage
  useEffect(() => {
  ...
  const handleSearch = (e) => {
    ...
    setQuery(val);
    localStorage.setItem("searchTerm", val); // ✅ save before reload
    onSearch(val);
  };
  ...
  return (
    <nav className="w-full max-w-6xl bg-white/90 backdrop-blur-md shadow-md rounded-full mt-4 px-6 py-3 flex justify-between items-center">
      ...
      >
        ...
        <h1 className="text-xl font-bold text-red-700">Kube-Netra</h1>
      </div>
